...
from PyQt6.QtCore import Qt, pyqtSignal, QThread, QPropertyAnimation, QEasingCurve, QEvent, QObject, QTimer
...
def show_exception_box(exc_type, exc_value, exc_tb):
    """Global exception handler to catch unhandled errors and display them."""
...
class AnimatedButton(QPushButton):
    """A custom button with hover and press animations for a more interactive feel."""
    ...
    def mousePressEvent(self, event):
        # Briefly darken on press for tactile feedback
...
class SpinnerLabel(QLabel):
    """Animated spinner to indicate progress."""
    def __init__(self):
        super().__init__()
        try:
            ...
            if os.path.exists(spinner_path):
                ...
                if not self.movie.isValid():
                    print(f"Warning: spinner.gif is invalid at {spinner_path}")
                    self.setText("...")
                else:
                    self.setMovie(self.movie)
            else:
                print(f"Warning: spinner.gif not found at {spinner_path}")
                self.setText("...") # Fallback text
        except Exception as ex:
            print("SpinnerLabel error:", ex)
            self.setText("...")
    ...
    def stop(self):
        if hasattr(self, 'movie') and self.movie.isValid():
            try:
                ...
                print("Spinner stop error:", ex)
        self.setVisible(False)

class CustomDropLabel(QLabel):
    """ A QLabel widget that accepts drag-and-drop with interactive animations. """
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            ...
            show_exception_box(type(ex), ex, ex.__traceback__)
    ...
    def set_image(self, file_path, domain):
        try:
            if file_path:
                ...
                if pixmap.isNull():
                    QMessageBox.critical(self, "Invalid Image", "The selected image could not be loaded. Please select a valid image file.")
                    return
